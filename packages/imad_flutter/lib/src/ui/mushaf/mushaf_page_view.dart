...
class MushafPageView extends StatefulWidget {
  ...
  /// Called when the user taps the page (empty area).
  final VoidCallback? onTap;

  /// Called when the user long-presses a verse.
  final void Function(int chapterNumber, int verseNumber)? onVerseLongPress;

  /// If set, the verse with this key (chapterNumber * 1000 + verseNumber)
  /// will be highlighted when the page first loads.
  final int? initialSelectedVerseKey;
...
class MushafPageViewState extends State<MushafPageView> {
  ...
  void initState() {
    super.initState();
    _selectedVerseKey = widget.initialSelectedVerseKey;
  ...
  /// If [highlightVerseKey] is provided (chapter * 1000 + verse),
  /// that verse will be highlighted on the target page.
  void goToPage(int page, {int? highlightVerseKey}) {
    final clampedPage = page.clamp(1, QuranDataProvider.totalPages);
    _preserveSelection = highlightVerseKey != null;
    setState(() {
      _currentPage = clampedPage;
      _selectedVerseKey = highlightVerseKey;
    });
        ...
        children: [
          Expanded(
            child: GestureDetector(
              ...
              child: Stack(
                children: [
                  // Main page view (RTL page order — force LTR so swipe
                  // direction is consistent: swipe left = next page)
                  Directionality(
                      ...
                      return QuranPageWidget(
                        ...
                        onVerseLongPress: widget.onVerseLongPress == null
                            ...
                            : (chapter, verse) {
                                ...
                                widget.onVerseLongPress!(chapter, verse);
                              },
                      );
                    },
                  ),
